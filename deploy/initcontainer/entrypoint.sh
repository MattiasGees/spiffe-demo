...
[default]
credential_process = /usr/bin/aws-spiffe-workload-helper x509-credential-process --profile-arn ${AWS_PROFILE_ARN} --trust-anchor-arn ${AWS_TRUSTANCHOR_ARN} --role-arn ${AWS_ROLE_ARN}
EOL
else
cat > /tmp/aws/config <<EOL
[default]
